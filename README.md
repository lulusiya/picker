...
6. 点“推送”（或在输入框按 `Enter`，`Shift+Enter` 换行）可把当前选取与修改要求**立即注入正在监听的会话**。只有在真的有 agent 在监听时，这个按钮才会出现（见下节）。
...
|---|---|---|---|
| **复制** | 进剪贴板 | 无 | 全部 |
| **排队** | 写进 `.picker/`，agent 下次开口时送达 | agent 有 prompt 钩子 | 装了 `picker-hook` 的 agent |
| **推送** | 注入**正在运行**的会话，不等它开口 | host 提供注入通道 | **只有 Pi** |
| **拉取** | agent 主动问 | MCP 客户端 | 任何 MCP 客户端 |

关键区别：**推送需要 host 提供一个“从外部注入并触发一轮”的通道**。Pi 提供（扩展 API 的 `sendMessage(..., { triggerTurn: true })`），Claude Code 和 Codex 的钩子只能在生命周期事件上触发，**没有这个入口**。所以对它们来说“推送”实际是**排队直到它下次开口**——你还是得在 agent 里发一条消息。

因此面板不会说谎：

- 有 agent 通过心跳（`.picker/listeners/<agent>.json`，每 2 秒刷新）报活→ 路由按钮出现绿点，推送按钮出现并标注“推送给 pi”
- 没有心跳 → **推送按钮直接不显示**；点击也不可能得到一个假成功提示
- 心跳超过 10 秒未刷新（agent 关掉了）→ 能力自动收回

所以任何 host 只要实现“心跳 + 轮询 `push.json`”，就自动从排队升到推送层——插件里没有硬编码任何 agent 名单。
